...
def detect_face(img, detector_backend = 'opencv', grayscale = False, enforce_detection = True, align = True):
	...
	#----------------------------------------------
	# Check if the image is a photo of a photo
	if detect_photo_of_photo(img):
		raise ValueError("Detected a photo of a photo. Please provide an original photo for attendance.")
...
def detect_photo_of_photo(img):
    """
    Detects if the image contains rectangular frames that could indicate a photo of a photo.
    Returns True if suspicious frames are detected, False otherwise.
    """
    ...
    for contour in contours:
        ...
        # Check if the contour is a significant portion of the image
        # and has a reasonable aspect ratio (between 0.5 and 2.0)
        if (area > 0.1 * img_area and 0.5 < aspect_ratio < 2.0):
            # Check if the rectangle is not too close to the image edges
            margin = 0.1
            if (x > margin * img.shape[1] and 
                y > margin * img.shape[0] and 
                x + w < (1 - margin) * img.shape[1] and 
                y + h < (1 - margin) * img.shape[0]):
                return True
