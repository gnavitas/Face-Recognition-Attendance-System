...
class Window(QDialog):
    def __init__(self):
        ...
        self.loginButton.clicked.connect(lambda: self.handleAttendance("Log-in"))
        self.breakInButton.clicked.connect(lambda: self.handleAttendance("Break-in"))
        self.breakOutButton.clicked.connect(lambda: self.handleAttendance("Break-out"))
        self.signOutButton.clicked.connect(lambda: self.handleAttendance("Log-out"))
        
        # Fix logButton connection and style
        self.logButton.setCursor(Qt.PointingHandCursor)
        self.logButton.clicked.connect(self.runSlot2)  # Connect to admin login
    ...
    def handleAttendance(self, attendance_type):
        """Handle attendance button clicks"""
        # Set the global attendance type
        global_ini.ATTENDANCE_TYPE = attendance_type
        
        # For Break-in, check database sync first
        if attendance_type == "Break-in":
            try:
                ...
                if server_sum != client_sum:
                    ...
                    if x != QMessageBox.Ignore:
                        return  # Don't proceed if sync is required
            except Exception as e:
                logging.error(f"Sync check error: {e}")
                # Continue anyway if sync check fails
        ...
        self._new_window.returnToStartSignal.connect(self.backMenu)  # Connect to the signal

    def backMenu(self):
        """Return to the start menu"""
        if self._new_window:
            self._new_window.hide()  # Hide the main window
            self._new_window = None  # Clear the reference
    ...
    def update_datetime(self):
        ...
        sql_date = "SELECT DATE_FORMAT(NOW(), '%W, %d %M %Y') as date, DATE_FORMAT(NOW(), '%H:%i:%s') as time"
        global_ini.mycursor.execute(sql_date)
        time_result = global_ini.mycursor.fetchone()
        date_str = time_result['date']
        time_str = time_result['time']
